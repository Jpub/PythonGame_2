...
def game_main():
    ...
    if index == 0:  # 타이틀 로고
        draw_txt("애니멀", 312, 240, 100, "lime", "TITLE")
        ...
        if drop_neko() == False:
            index = 3
        draw_neko()
    elif index == 3:  # 나란히 있는가?
        check_neko()
        draw_neko()
        index = 4
    elif index == 4:  # 나란히 있는 고양이 삭제
        sc = sweep_neko()
        ...
        if timer == 50:
            ...
            index = 0
    cvs.delete("INFO")
    draw_txt("SCORE " + str(score), 160, 60, 32, "cyan", "INFO")
    draw_txt("HISC " + str(hisc), 450, 60, 32, "yellow", "INFO")
    if tsugi > 0:
        cvs.create_image(752, 176, image=img_neko[tsugi], tag="INFO")
    root.after(100, game_main)


root = tkinter.Tk()
root.title("블록 낙하 퍼즐 '애니멀'")
...
img_neko = [
    None,
    tkinter.PhotoImage(file="animal1.png"),
    tkinter.PhotoImage(file="animal2.png"),
    tkinter.PhotoImage(file="animal3.png"),
    tkinter.PhotoImage(file="animal4.png"),
    tkinter.PhotoImage(file="animal5.png"),
    tkinter.PhotoImage(file="animal6.png"),
